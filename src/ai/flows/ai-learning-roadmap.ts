...
const LessonSchema = z.object({
  ...
  topics: z.array(z.string()).describe("A list of at least 3-5 specific topics covered, providing a clear breakdown of lesson content. Each topic string MUST be in the `interfaceLanguage`. These strings should be descriptive and MUST be prefixed with a category like 'Грамматика:', 'Лексика:', 'Чтение:', 'Письмо:', or 'Аудирование:'. For example: 'Грамматика: Изучение немецкого алфавита', 'Лексика: Практика слов по теме семья'. Every lesson MUST include at least one 'Лексика:' (Vocabulary) topic."),
...
const GeneratePersonalizedLearningRoadmapOutputSchema = z.object({
  ...
  lessons: z.array(LessonSchema).min(18).describe("An array of lessons, structured sequentially from A0 to C2. Ensure comprehensive coverage for the `targetLanguage` across all CEFR levels. Each lesson should aim to integrate various skills (grammar, vocabulary, listening, reading, writing) in a thematic or functional context where possible."),
...
4.  **Lesson Structure:**
    ...
    *   Each topic string MUST start with a category prefix, like 'Грамматика:', 'Лексика:', 'Чтение:', 'Письмо:', or 'Аудирование:'. DO NOT include 'Говорение:' (Speaking) topics.
